...
const validateInput = () => {
  ...
  if (!password.value) {
    ...
    if (password.value.length < 6) {
      ...
      isValid = false;
    } else if (/(\w)\1/.test(password.value)) {
      passwordError.value =
...
const handleSubmit = async () => {
  ...
  try {
    await userStore.registerUser(userName.value, email.value, password.value);

    toastStore.triggerSuccess(`Welcome ${userName.value}!`);
...
<template>
  ...
  >
    ...
    <div class="pb-8 text-sm font-light">
      Sign up for an account on CodeWave.
    </div>
